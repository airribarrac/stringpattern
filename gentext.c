	...
	{ gettimeofday (&t,NULL);
     ...
     for (i=0;i<n;i++)
       { Seed = ((test = ACMa * lo - ACMr * hi) > 0) ? test : test + ACMm;
         A[i] = offset+65 + ((double)Seed) * s/ACMm;
       }
   }
...
main (int argc, char **argv)
   ...
   { unsigned long n;
     ...
     if (argc < 3)
        { fprintf (stderr,"Usage: %s <file> <n> <sigma> <offset>\n"
		          " generates <file> with <n> kilobytes of text"
			  " uniformly distributed in [1,<sigma>] <offset>\n",argv[0]);
          exit(1);
        }
     offset = atoi(argv[4]);
     n = atoi(argv[2]);
